...
public class VideoConverter {
    ...
    public VideoConverter() {
        useSoftware = DeviceUtils.useSoftware();
    }
...
//    }
    ...
    private void initialization(String sourcePath, String outputPath, CompressProgressListener listener) {
        ...
        if (extractor != null) {
            ...
            MLog.w(TAG, "original data : bit rate = " + audioBitRate + " , smapleRate = " + sampleRate + " , channelCount = " + channelCount);
            extractor.release();
        }
    ...
    public boolean extractDecodeEditEncodeMux(String sourcePath, String outputPath, CompressProgressListener listener) throws Exception {
        ...
        if (videoCodecInfo == null) {
            ...
            MLog.e(TAG, "Unable to find an appropriate codec for " + OUTPUT_VIDEO_MIME_TYPE);
            return false;
        }
        MLog.d(TAG, "odevideo found cc: " + videoCodecInfo.getName());

        MediaCodecInfo audioCodecInfo = selectCodec(OUTPUT_AUDIO_MIME_TYPE);
        if (audioCodecInfo == null) {
            // Don't fail CTS if they don't have an AAC codec (not here, anyway).
            MLog.e(TAG, "Unable to find an appropriate codec for " + OUTPUT_AUDIO_MIME_TYPE);
            return false;
        }
        MLog.d(TAG, "audio found codec: " + audioCodecInfo.getName());
...
//                assertTrue("missing video track in test video", videoInputTrack != -1);
                ...
                MLog.d(TAG, "video format: " + outputVideoFormat);
...
//                outputAudioFormat.setInteger(MediaFormat.KEY_AAC_PROFILE, OUTPUT_AUDIO_AAC_PROFILE);
            ...
            MLog.d(TAG, "releasing extractor, decoder, encoder, and muxer");
            ...
            try {
                ...
                MLog.e(TAG, "error while releasing videoExtractor");
                e.printStackTrace();
                if (exception == null) {
                    exception = e;
                }
            ...
            try {
                ...
                MLog.e(TAG, "error while releasing audioExtractor");
                e.printStackTrace();
                if (exception == null) {
                    exception = e;
                }
            ...
            try {
                ...
                MLog.e(TAG, "error while releasing videoDecoder");
                e.printStackTrace();
                if (exception == null) {
                    exception = e;
                }
            ...
            try {
                ...
                MLog.e(TAG, "error while releasing outputSurface");
                e.printStackTrace();
                if (exception == null) {
                    exception = e;
                }
            ...
            try {
                ...
                MLog.e(TAG, "error while releasing videoEncoder");
                e.printStackTrace();
                if (exception == null) {
                    exception = e;
                }
            ...
            try {
                ...
                MLog.e(TAG, "error while releasing audioDecoder");
                e.printStackTrace();
                if (exception == null) {
                    exception = e;
                }
            ...
            try {
                ...
                MLog.e(TAG, "error while releasing audioEncoder");
                e.printStackTrace();
                if (exception == null) {
                    exception = e;
                }
            ...
            try {
                ...
                MLog.e(TAG, "error while releasing muxer");
                e.printStackTrace();
                if (exception == null) {
                    exception = e;
                }
            ...
            try {
                ...
                MLog.e(TAG, "error while releasing inputSurface");
                e.printStackTrace();
                if (exception == null) {
                    exception = e;
                }
    ...
    private MediaCodec createVideoDecoder(MediaFormat inputFormat, Surface surface) throws IOException {
        ...
        MediaCodec.Callback callback = new MediaCodec.Callback() {
            public void onError(MediaCodec codec, MediaCodec.CodecException exception) {
                MLog.e(TAG, "video decoder error() ");
            }
            public void onOutputFormatChanged(MediaCodec codec, MediaFormat format) {
                mDecoderOutputVideoFormat = codec.getOutputFormat();
                MLog.d(TAG, "video decoder: onOutputFormatChanged(): "
            ...
            public void onInputBufferAvailable(MediaCodec codec, int index) {
                ...
                while (!mVideoExtractorDone) {
                    ...
                    if (mVideoExtractorDone) {
                        MLog.d(TAG, "video extractor: EOS");
                        codec.queueInputBuffer(
                                index,
                                0,
                                0,
            ...
            public void onOutputBufferAvailable(MediaCodec codec, int index, MediaCodec.BufferInfo info) {
                ...
                if ((info.flags & MediaCodec.BUFFER_FLAG_CODEC_CONFIG) != 0) {
                    MLog.d(TAG, "video decoder: codec config buffer");
                ...
                try {
                    ...
                    if (render) {
                        mInputSurface.makeCurrent();
                        MLog.d(TAG, "output surface: await new image");
                        mOutputSurface.awaitNewImage();
                        // Edit the frame and send it to the encoder.
                        MLog.d(TAG, "output surface: draw image");
                        mOutputSurface.drawImage();
                        mInputSurface.setPresentationTime(
                                info.presentationTimeUs * 1000);
                        MLog.d(TAG, "input surface: swap buffers");
                        mInputSurface.swapBuffers();
                        MLog.d(TAG, "video encoder: notified of new frame");
                        mInputSurface.releaseEGLContext();
                    }
                    if ((info.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) {
                        MLog.d(TAG, "video decoder: EOS");
...
//         not essential to the actual transcoding.
    ...
    private MediaCodec createVideoEncoder(
        ...
        encoder.setCallback(new MediaCodec.Callback() {
            public void onError(MediaCodec codec, MediaCodec.CodecException exception) {
                MLog.e(TAG, "video encoder error() ");
            }
            public void onOutputFormatChanged(MediaCodec codec, MediaFormat format) {
                MLog.d(TAG, "video encoder: onOutputFormatChanged()");
            ...
            public void onOutputBufferAvailable(MediaCodec codec, int index, MediaCodec.BufferInfo info) {
                MLog.d(TAG, "video encoder: onOutputBufferAvailable()");
                if (isCancel) return;
                muxVideo(index, info);
            }
        });
    ...
    private MediaCodec createAudioDecoder(MediaFormat inputFormat) throws IOException {
        ...
        decoder.setCallback(new MediaCodec.Callback() {
            public void onError(MediaCodec codec, MediaCodec.CodecException exception) {
                MLog.e(TAG, "audio decoder error() ");
            }
            public void onOutputFormatChanged(MediaCodec codec, MediaFormat format) {
                mDecoderOutputAudioFormat = codec.getOutputFormat();
                MLog.d(TAG, "audio decoder: onOutputFormatChanged(): "
            ...
            public void onInputBufferAvailable(MediaCodec codec, int index) {
                ...
                while (!mAudioExtractorDone) {
                    ...
                    if (mAudioExtractorDone) {
                        MLog.d(TAG, "audio extractor: EOS");
                        codec.queueInputBuffer(
                                index,
                                0,
                                0,
            ...
            public void onOutputBufferAvailable(MediaCodec codec, int index, MediaCodec.BufferInfo info) {
                MLog.d(TAG, "audio decoder: onOutputBufferAvailable()");
                if (isCancel) return;
                ByteBuffer decoderOutputBuffer = codec.getOutputBuffer(index);
                if ((info.flags & MediaCodec.BUFFER_FLAG_CODEC_CONFIG) != 0) {
                    MLog.d(TAG, "audio decoder: codec config buffer");
                    codec.releaseOutputBuffer(index, false);
                    return;
                }
                MLog.w(TAG, "audio decoder: onOutputBufferAvailable() presentationTimeUs = "
    ...
    private MediaCodec createAudioEncoder(MediaCodecInfo codecInfo, MediaFormat format) throws IOException {
        ...
        encoder.setCallback(new MediaCodec.Callback() {
            public void onError(MediaCodec codec, MediaCodec.CodecException exception) {
                MLog.e(TAG, "audio encoder error() ");
            }
            public void onOutputFormatChanged(MediaCodec codec, MediaFormat format) {
                MLog.d(TAG, "audio encoder: onOutputFormatChanged()");
            ...
            public void onInputBufferAvailable(MediaCodec codec, int index) {
                MLog.d(TAG, "audio encoder: onInputBufferAvailable() buffer: " + index);
                if (isCancel) return;
                mPendingAudioEncoderInputBufferIndices.add(index);
                tryEncodeAudio();
            }
            public void onOutputBufferAvailable(MediaCodec codec, int index, MediaCodec.BufferInfo info) {
                MLog.d(TAG, "audio encoder: onOutputBufferAvailable() ");
                if (isCancel) return;
                muxAudio(index, info);
            }
        });
    ...
    private void tryEncodeAudio() {
        ...
        try {
            ...
            MLog.d(TAG, "audio decoder: pending buffer for time " + presentationTime);
            MLog.w(TAG, "tryEncodeAudio() enIndex = " + encoderIndex + " , deIndex = " + decoderIndex);
            if (size >= 0) {
...
//        MLog.d(TAG, "muxVideo: presentationTimeUs = "
//                + info.presentationTimeUs + " , duration = " + duration);
    ...
    private void muxAudio(int index, MediaCodec.BufferInfo info) {
        ...
        {
            MLog.d(TAG, "muxAudio: presentationTimeUs = " + info.presentationTimeUs);
        }
        if (info.size != 0) {
            mMuxer.writeSampleData(
    ...
    private int getAndSelectVideoTrackIndex(MediaExtractor extractor) {
        for (int index = 0; index < extractor.getTrackCount(); ++index) {
            {
                MLog.d(TAG, "format for track " + index + " is "
...
//        } finally {
//            if (mVideoDecoderHandlerThread != null) {
//                Log.w(TAG, "terminating the video decoder handler thread");
